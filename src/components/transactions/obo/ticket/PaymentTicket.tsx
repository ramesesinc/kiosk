  ...
  return (
    <>
      <div className="hidden">
        <PaymentTicketPrint ref={componentRef} QRCode={<QRCode value={combinedData} size={80} />} addr={payerAddress} appNo={oboBill.appno} payerName={payerName} seriesno={seriesno} total={<Currency amount={oboBill.amount} />} lguName={lguName} logo={lguLgo} />
      </div>
                      ...
                      <tbody>
                        {headers.map((label, index) => (
                          <tr key={index} className="flex gap-x-20">
                            ...
                            <td className="text-start text-[15px] leading-6 font-semibold  font-mono">{[` ${payerName}`, payerAddress ? ` ${payerAddress}` : "", " OSCP Billing and Payment", <Currency key={`currency-${index}`} amount={oboBill.amount} currency="Php" />, oboBill.appno ? ` ${oboBill.appno}` : ""][index]}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
