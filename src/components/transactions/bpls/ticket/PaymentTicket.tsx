...
const PaymentTicket: React.FC<PaymentTicketProps> = ({ isOpen, onClose, showClose, txntype, seriesno }) => {
  ...
  const headers = ["trxn date", "payer", "address", "particulars", "total", "bin no"];
  ...
  return (
    <>
      <div className="absolute top-0 left-0 bg-green-500 hidden">
        <PaymentTicketPrint ref={componentRef} QRCode={<QRCode value={combinedData} size={100} />} addr={payerAddress} total={bill.amount.toLocaleString()} binNo={bill.info.bin} appDate={bill.info.appdate} payerName={payerName} seriesno={seriesno} lguName={lguName} logo={lguLgo} />
      </div>
                      ...
                      <tbody>
                        {headers.map((label, index) => (
                          <tr key={index} className="flex gap-x-20">
                            ...
                            <td className="text-start text-[15px] leading-6 font-semibold font-mono">{[bill.info.appdate ? <CurrentDate /> : "", `${payerName}`, payerAddress ? `${payerAddress}` : "", "Business Billing and Payment", bill.amount ? `${bill.amount.toLocaleString()}` : "", combinedData ? `${bill.info.bin}` : ""][index]}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
