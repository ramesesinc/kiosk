...
const PaymentTicket: React.FC<PaymentTicketProps> = ({ isOpen, onClose, showClose, rpttxntype, seriesno }) => {
  ...
  const { taxBill, billToQtr, billToYear, payerName, payerAddress } = useTaxBillingContext();
  const combinedData = `${rpttxntype}&billtoqtr=${billToQtr}&billtoyear=${billToYear}&paidby=${payerName}&paidbyaddress=${payerAddress}`;
  const headers = ["trxn date", "payer", "address", "particulars", "total", "tax no"];
  ...
  return (
    <>
      <div className="hidden">
        <PaymentTicketPrint ref={componentRef} QRCode={<QRCode value={combinedData} size={80} />} addr={payerAddress} appDate={taxBill.info?.billdate} total={<Currency amount={taxBill.amount} />} tdNo={taxBill.info.tdno} payerName={payerName} seriesno={seriesno} lguName={lguName} logo={lguLgo} />
      </div>
        ...
        <div className="w-[60%] relative flex flex-col items-center bg-white p-4 rounded-2xl z-10">
          ...
          {isPrinting ? (
            <div className="flex flex-col items-center justify-center pb-20 gap-10">
              ...
              <Title text={"Printing Ticket"} classname="!text-[25px] !leading-[0px]" />
              <Subtitle text={" Please wait while your ticket is being printed."} classname="text-[20px] !leading-[0px]" />
            </div>
          ) : (
            <div className="m-8">
              <div className="w-full flex flex-col gap-y-10">
                {ticketInfo.map((item, index) => (
                  <div key={index} className="flex w-full flex-wrap items-center justify-between">
                    <div className="flex container mx-auto">
                      <Image src={lguLgo} alt={""} width={item.logo.width} height={0} loading="eager" />
                      <div className="flex flex-col justify-center mx-4">
                        <Title text={item.header.title} classname="uppercase text-[24px] leading-6" />
                        <Title text={lguName} classname="uppercase text-[24px] leading-6" />
                      </div>
                    </div>
                  </div>
                ))}
                <div className="flex gap-x-10 justify-center">
                  <div className="relative">
                    <QRCode className="break-words" value={combinedData} size={90} />
                  </div>
                ...
                <div className="flex flex-col gap-2">
                  <Title text="present this receipt to the collector" classname="uppercase" textSize="text-xl" />
                  <div className="pl-6">
                    <table>
                      <tbody>
                        {headers.map((label, index) => (
                          <tr key={index} className="flex gap-x-20">
                            <td className="text-start text-[15px] leading-6 capitalize w-24">{label}</td>
                            <td className="text-start text-[15px] leading-6 font-semibold">{[taxBill.info?.billdate ? <CurrentDate /> : "", ` ${payerName}`, payerAddress ? ` ${payerAddress}` : "", " Real Tax Billing and Payment", <Currency key={`currency-${index}`} amount={taxBill.amount} currency="Php" />, taxBill.info?.tdno ? ` ${taxBill.info?.tdno}` : ""][index]}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
