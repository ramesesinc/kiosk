import React, { createContext, useContext, useRef, useState, Dispatch, SetStateAction } from "react";

interface KeyboardContextProps {
  activeInput: React.RefObject<HTMLInputElement> | null;
  setActiveInput: Dispatch<SetStateAction<React.RefObject<HTMLInputElement> | null>>;
...
function KeyboardContextProvider({ children }: { children: React.ReactNode }) {
  const [activeInput, setActiveInput] = useState<React.RefObject<HTMLInputElement> | null>(null);
  const [keyboardType, setKeyboardType] = useState<"alpha" | "numeric">("alpha");
